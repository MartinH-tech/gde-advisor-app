...
function DraftsScreen({ navigation }: DraftsScreenProps) {
...
function SchoolVisitScreen({ setPreviewContent }: SchoolVisitScreenProps) {
  ...
  const [previewContent, _setPreviewContent] = React.useState('');
  const [lastSaved, setLastSaved] = React.useState<string | null>(null);
  ...
  const saveDraft = async () => {
    const draft = {
      ...
      syllabus: syllabus || {},
      sba: sba || {},
      findings: typeof findings === 'string' ? findings : '',
      challenges: typeof challenges === 'string' ? challenges : '',
  ...
  const loadDraft = async () => {
    try {
      ...
      if (saved) {
        ...
        setStatus(typeof draft.status === 'string' ? draft.status : '');
        setSchool(typeof draft.school === 'string' ? draft.school : '');
        setSubject(typeof draft.subject === 'string' ? draft.subject : '');
        setSyllabus(draft.syllabus || syllabus);
        setSba(draft.sba || sba);
        setFindings(typeof draft.findings === 'string' ? draft.findings : '');
        setChallenges(typeof draft.challenges === 'string' ? draft.challenges : '');
        setRecommendations(typeof draft.recommendations === 'string' ? draft.recommendations : '');
      ...
      // fallback: reset all draft state
      setPurpose([]);
      setStatus('');
      setSchool('');
      setSubject('');
...
${schoolChallenges}
  ...
  const submitPriorityReport = async () => {
    ...
    // Image signature conversion removed for phase 2
